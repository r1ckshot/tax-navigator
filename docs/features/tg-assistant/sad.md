...
### Критичний потік 1: тижневий цикл збору — авторизовані чати, дедуп при надолуженні (AC-01, AC-02, AC-09)

<!-- Написано /sdlc-complete-sequence-diagrams (невендорений, запуск «як є»): generic-учасники
з фіксованого словника скіла (<client>/<service>/<data-store>/<external-system>), НЕ конкретні
модулі з §5 — свідомий відхід від конвенції sad-forge, за якою написані інші секції цього файлу.
Async-класифікація за ключовим словом «тижневий» (US-01) — idempotency-key/retry/DLQ додані
механічно, без окремого запитання, чи потрібні тут ретраї. -->
...
sequenceDiagram
    ...
    Note over C,S: Trigger: щотижневий cron-запуск циклу збору (US-01)
    C->>S: Запускає черговий цикл збору
    S->>S: перевіряє ключ ідемпотентності циклу (тиждень+чат; пропускає вже оброблені питання при надолуженні, AC-09)
    S->>D: Читає список чатів, де дослідник учасник, і позначку останнього обробленого тижня
    D-->>S: Список авторизованих чатів + маркер catch-up
    S->>S: виключає чати, до яких дослідник ніколи не приєднувався (AC-02) — вони не потрапляють до списку обходу
    loop Кожен авторизований чат
        S->>X: Читає нові повідомлення чату з позначки останнього тижня
        X-->>S: Нові повідомлення
    end
    Note over S,X: retry 3 рази з експоненційним backoff при недоступності чату/API
    alt вичерпано ретраї для чату
        S->>D: Позначає dead-letter для цього чату (буде явно названий у звіті як недоступний, AC-08)
        Note over S,D: dead-letter після 3 невдалих спроб поспіль
    end
    S->>D: Записує зібрані повідомлення й оновлює маркер тижня (дедуп проти вже зібраних, AC-09)
    Note over S,D: persists messages, cycle_state (інформує generate-data-model)
    D-->>S: ok
    Note over C,S: Postcondition: усі авторизовані чати оброблені рівно раз за тиждень, неавторизовані — відсутні в результаті (AC-02), пропущений тиждень надолужено без дублів (AC-09)
```

**Coverage — complete-sequence-diagrams (§4 US / §5 AC проти цього §6).** До цього прогону
§6 був незаповненим шаблоном з 6.4 (плейсхолдери `<назва>`/`User`/`API`/`Service`/`DB`,
жодного реального потоку) — тому таблиця стартує з майже суцільного Missing: це стан
вхідних даних, а не пропуск цього прогону complete-sequence-diagrams.

| US | AC | Стан | Де |
|---|---|---|---|
| US-01 | AC-01 happy path (збір повідомлень) | Covered | Потік 1 |
| US-01 | AC-02 authorization (чужий чат) | Covered | Потік 1 |
| US-02 | AC-03 фільтр органічних питань | Missing | — |
| US-02 | AC-04 дедуп мультитемних повідомлень | Missing | — |
| US-03 | AC-05 мітка «покрито»/«біла пляма» | Missing | — |
| US-03 | AC-06 історичні мітки не переписуються | Missing | — |
| US-04 | AC-07 тижневий звіт | Missing | — |
| US-05 | AC-08 явна недоступність чату | Trivial | dead-letter гілка потоку 1, без окремого «список усіх недоступних» |
| US-05 | AC-09 надолуження без дублів | Covered | Потік 1 |
| US-05 | AC-10 catch-up нового чату — вікно N тижнів | Missing | — |

Свідомо не закрито в цьому прогоні — простий рівень уроку 6.5 малює один потік, не весь хвіст.
