...
- [x] **AC-01:** Given дослідник уже учасник обраних чатів, when настає тижневий момент розкладу, then система безнаглядно збирає нові повідомлення з цих чатів і готує їх до розмітки.
- [x] **AC-02:** Given чат, до якого дослідник ніколи не приєднувався, when цикл формує список чатів для обходу, then система ніколи не включає цей чат і не показує його в звіті.
- [x] **AC-08:** Given дослідник під час циклу втратив доступ до раніше доступного чату (виключений, чат видалено чи закрито) або читання частини чатів не вдалося, when цикл формує звіт, then система явно називає, до яких чатів доступ втрачено чи не вдалося прочитати, а не показує правдоподібний нуль.
- [x] **AC-09:** Given попередній цикл пропущено, when наступний цикл запускається, then він надолужує пропущений тиждень, не дублюючи вже зібрані раніше питання.
- [x] **AC-10:** Given дослідник щойно приєднався до нового чату з історією довшою за N тижнів, when наступний цикл обробляє цей чат, then система підтягує лише останні N тижнів історії і явно позначає межу вікна в звіті, не заглиблюючись автоматично далі.

## Checklist (atomic steps for impl-agent)

- [x] Step 1 — Реалізувати `collector.ts`: MTProto-читання нових повідомлень чату з позначки останнього тижня (ADR-0001), лише для чатів зі списку `chats` — AC-02 гейтиться на рівні запиту (чужий чат фізично відсутній у таблиці). Частково скриптовано: сам gate-фільтр (без MTProto-читання) — `research/tg-assistant/chatFilter.ts` + `chatFilter.test.ts` (6 тестів, лекція 7.7, `/tdd --review-tests`). Живе MTProto-читання — `research/tg-assistant/telegram.ts` + `collector.ts` (2026-09-14, урок 11.2).
- [x] Step 2 — Додати чергу по чатах з експоненційним backoff, що читає конкретне значення X із `FLOOD_WAIT_X` (ADR-0002); при вичерпаних ретраях (3 поспіль) писати рядок у `cycle_chat_failures.reason` замість тихого нуля (AC-08). Частково скриптовано: рішення "retry чи dead-letter" (без самої черги/мережевих викликів) — `research/tg-assistant/retryQueue.ts` + `retryQueue.test.ts` (5 тестів, лекція 7.7, `/tdd`). Черга по чатах і реальний FLOOD_WAIT — `collector.ts` (2026-09-14, урок 11.2).
...
- [x] Step 4 — Для чату з `chats.created_at` новішим за N тижнів (конфігурований дефолт — PRD §8 відкрите питання, зараз 4) обмежити глибину читання вікном N тижнів і явно позначити межу вікна в даних, що підуть у звіт (AC-10). Скриптовано: `research/tg-assistant/window.ts` + `window.test.ts` (6 тестів, лекція 7.7, ручний TDD без агентів).
...
- [x] AC-09 anti-regression: повторний прогін циклу для того самого `week_of` не створює других `messages`-рядків — `UNIQUE(chat_id, telegram_message_id)` ловить дубль на catch-up.
- [x] Lint + типи clean (per SAD §2 Constraints). Лінта в `research/` немає; типи — `tsc -p research/tg-assistant/tsconfig.json`.
- [x] Integration test покриває всі ACs цієї story.
- [x] PR linked back to this story file: PR #53 закриває issue #52, а issue посилається на `tasks/S-1-tg-assistant.md`.
