...
2026-08-27** — план складних рівнів і рунбук простих готові, уроки ще не
робились ([docs/capstones/m11.md](capstones/m11.md)); капстоуна в модулі немає,
11.7 — фінальний урок без завдання. Це останній модуль курсу. Пріоритет поза
курсом лишається реальний білд: T1+T2 `tg-assistant` за `tasks/tracker.md`.
...
**M11 Production у роботі: 11.1 закрито 2026-09-12, наступний — 11.2. Це
...
- [x] **Три уроки везуть справжні продуктові шматки.** 11.2 — колектор
  ...
  [POLISH.md](POLISH.md)
- [x] **Пастка 11.2 знята, а не обійдена.** Чеклісту потрібна «база в окремому
  сховищі», і спершу здавалось, що предмета немає: `SPEC.md` §Non-goals каже «не
  бекенд». Але `tg-assistant` має власну БД — `data-model.md` обрав SQLite, і
  staged-міграції прогнані roundtrip ще в M6. Тобто предмет буквальний, а
  підміна станом монітора не потрібна
