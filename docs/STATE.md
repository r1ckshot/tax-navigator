...
6 сценаріїв рахуються детерміновано на клієнті. 326 node-тестів + 17 UI зелені.
...
- [x] **Межа ПІСЛЯ моделі, якої бракувало:**
  ...
  `drift-reviewer` `Bash` немає взагалі. 23 кейси зелені, мутація (порожній
...
- [x] Гейт: 326 node-тестів (+24), `depcruise` 0 порушень, `lint.py` 11/11,
