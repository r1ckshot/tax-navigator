...
- [ ] **Живий прогін хука на `drift-reviewer` не зроблено.** Спавн агента двічі
  поспіль відмовив класифікатор auto-режиму (`Blocked by classifier`) — це не
  хук і не агент, а відомий запис в `environment-limits.md`. Обхід — Shift+Tab
  на `acceptEdits`. До того межа доведена лише ізольовано, 23 кейси
