...
5 сценаріїв рахуються детерміновано на клієнті. 107 node-тестів + 10 UI зелені.
...
### Наступний крок: F2 через плагін

Друга фіча тієї самої форми — сценарій **`działalność nierejestrowana`** —
робиться **через плагін**, і це предмет виміру, а не спосіб зекономити:

1. `/scaffold-scenario nierejestrowana` — веде весь зріз і звітує по 10 місцях реєстрації.
2. Числа — через `/scaffold-rule`. Ліміт уже звірено: **10,813.50 zł/квартал**
   (225% мінімалки 4,806, поквартально з 2026) — EVIDENCE §Нестабільності п. 3.
   Решта (PIT за скалею, відсутність ZUS, KUP) — окремими `rule_id`.
3. Тести — скіл `scenario-tests` (еталон від центру смуги + мутаційна перевірка).
4. Здача — скіл `feature-ship`.
5. Фіксувати в CAPSTONE_LOG: час, кількість вільних промптів, **спрацювання хука
   `layer-boundary`**, і де плагін НЕ допоміг.

Гілку від `master` брати після мерджу цієї. PR #2 в описі має явно називати
команди плагіна.
