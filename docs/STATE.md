...
- [x] Рішення про курс UAH→PLN — не конвертуємо (DECISIONS 2026-07-29)
- [x] `fop.ts` віддає `foreignBurden`: ЄП+ВЗ = 6% доходу в zł (частка від валюти
  не залежить) + мін. ЄСВ 1,902.34 грн окремим полем; `rangeMonthly` лишається
  `null` свідомо
- [x] Еталон у `benchmark.test.ts` із виведенням у назві + UI-тест на підписані валюти
- [x] `SPEC.md` критерій 6 і карта архітектури переформульовані

Лишилось із DoD:
- [ ] **Візуальний рев'ю Mike** — блок «Український бік» у картці ФОП ще ніхто не
  бачив очима (playwright/puppeteer у проєкті немає, скріншот зняти нічим)
...
**1. Показати Mike картку ФОП** (`/questionnaire` → результат), потім merge
`feat/fop-figures`. Це остання незакрита позиція гілки.
