...
Готово коли:
- [ ] Ставки ЄСВ і ВЗ на 2026 звірені з `zakon.rada.gov.ua`, кожна з посиланням на статтю
- [ ] Рядок у `docs/EVIDENCE.md`: `Верифіковано <дата>: <посилання>` (порядок із `evidence-numbers.md` — спершу джерело, потім код)
- [ ] Правило у `rules.2026.json` із `source_url` + `verified_at`
...
⚠️ Вікно доступу може закритись при наступному Rebuild Container (Akamai-ротація
не стосується `zakon.rada`, але перевірити `curl` на старті — за `preflight.md`).
