...
**Готово коли:**
- [ ] Крок 0: еталони JDG, UoP, інкубатора звіряються з центром смуги, не
      `rangeContains`; мутація (зсув на 1 zł усередині `calc/`) валить набір —
      доведено прогоном, а не заявлено
- [ ] Скрипт тягне з `zus.pl` складку для кожного профілю у фікстуру з `fetched_at`
      і URL; у `npm test` мережевих викликів нема
