...
- [x] **Простий рівень — клон у чисту теку, і він знайшов дірку.**
  `.claude/settings.json` (deny 44 / ask 3 / allow 33) приїжджає з деревом без
  сетапу, а от `.githooks/` приїжджає **неактивним**: `core.hooksPath` у свіжому
  клоні порожній, у `.git/hooks/` самі `.sample`. Тобто половина шару 2 лежала на
  диску мертвою, а рядок, що її вмикає, жив рівно в тій прозі, проти якої весь
  цей контур і написаний. Закрито `df5e68d` — `postCreateCommand` девконтейнера
